...
In the course, you are back with the your old friends the Taco Wagon team. It's time to improve their configuration with functions, meta-arguments, and ephemeral values. The `base_app` directory has the basic configuration, which we will copy over to a new directory and start making alterations.
